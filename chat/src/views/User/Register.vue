...
const handleRegiste = async () => {
  const { err, res } = await useHttp(userStore.register, registerLoading)
...
<template>
   ...
   <div class=" fixed w-full z-10 flex justify-center" v-if="error">
    <div role="alert" class="alert mt-2 alert-error   w-1/2">
      ...
      <button class="btn btn-sm btn-ghost" @click="error = ''"><QIcon/></button>
    </div>
   </div>
  ...
  <div class="hero bg-base-200 h-full">
    ...
    <div class="hero-content flex-col lg:flex-row-reverse">
      ...
      <div class="card bg-base-100 w-full max-w-sm shrink-0 shadow-2xl">
        <form @submit.prevent="handleRegiste" class="card-body">
          <div class="form-control">
            <label class="label">
              <span class="label-text">名字</span>
            </label>
            <input type="text" placeholder="username" class="input input-bordered !outline-none" required
          ...
          <div class="form-control">
            ...
            <input type="password" placeholder="password" class="input input-bordered !outline-none" required
              v-model="userStore.userFormData.password" minlength="6" />
          </div>
          <div class="form-control">
            <label class="label">
              <span class="label-text">邮箱(用于找回密码)</span>
            </label>
            <input type="email" placeholder="email" class="input input-bordered !outline-none"
          ...
          <div class="form-control mt-1">
            ...
            <button class="btn flex-1 rounded-full btn-md" @click="router.push('/login')">
              去登录
            </button>
          </div>
        </form>
